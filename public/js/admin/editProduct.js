// Variant Indexing
const serverVariantIndexInput = document.getElementById("serverVariantIndex");
let variantIndex = serverVariantIndexInput ? parseInt(serverVariantIndexInput.value) : 0;
...
window.addVariant = () => {
    ...
    const html = `
        <div class="variant-row grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 bg-gray-50 p-4 rounded-lg border border-gray-200 relative">
          ...
          <input type="hidden" name="existingImages_${variantIndex}" value="[]">
          ...
          <div>
            ...
            <div id="new_preview_${variantIndex}" class="image-preview-container"></div>
          </div>
        </div>
      `;
...
window.removeExistingImage = (variantIdx, imageUrl) => {
    ...
    // Remove the visual element using window.event if available or standard finding
    // Better to use currentTarget if called from onclick="removeExistingImage(..., this)"
    // But EJS currently uses onclick="removeExistingImage('<%= index %>', '<%= img %>')"

    // We can use event.target since it's an onclick
    if (window.event) {
        const btn = window.event.target.closest('button') || window.event.target;
...
if (editProductForm) {
    editProductForm.addEventListener('submit', async function (e) {
        ...
        Swal.fire({
            ...
            text: 'Please wait while we process the images and save changes.',
